...
SimpleForm.setup do |config|
  ...
  config.button_class = 'my-2 bg-primary-500 hover:bg-primary-700 text-white font-bold text-sm py-2 px-4'
  ...
  config.error_notification_class = 'text-white px-6 py-4 border-0 relative mb-4 bg-red-400'
  ...
  config.wrappers :vertical_form, tag: 'div', class: 'mb-4' do |b|
    ...
    b.use :input, class: 'shadow appearance-none border border-gray-300 w-full py-2 px-3 bg-white focus:outline-none focus:ring-0 focus:border-blue-500 text-gray-400 leading-6 transition-colors duration-200 ease-in-out', error_class: 'border-red-500', valid_class: 'border-green-400'
  ...
  config.wrappers :vertical_boolean, tag: 'div', class: 'mb-4 flex items-start', error_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'flex items-center h-5' do |ba|
      ba.use :input, class: 'focus:ring-2 focus:ring-indigo-500:focus ring-offset-2 h-4 w-4 text-indigo-600 border-gray-300'
    end
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center', item_label_class: 'my-1 ml-3 block text-sm font-medium text-gray-400', tag: 'div', class: 'my-4' do |b|
    ...
    b.use :input, class: 'focus:ring-2 focus:ring-indigo-500 ring-offset-2 h-4 w-4 text-indigo-600 border-gray-300', error_class: 'text-red-500', valid_class: 'text-green-400'
  ...
  config.wrappers :vertical_file, tag: 'div', class: '' do |b|
    ...
    b.use :input, class: 'w-full text-gray-500 px-3 py-2 border', error_class: 'text-red-500 border-red-500', valid_class: 'text-green-400'
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-xs italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-grey-700 text-xs italic' }
  end
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'my-4', error_class: 'f', valid_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      # ba.use :input, class: 'flex w-auto w-auto text-gray-500 text-sm border-gray-300 p-2', error_class: 'text-red-500', valid_class: 'text-green-400'
      ba.use :input, class: 'flex w-auto w-auto shadow appearance-none border border-gray-300 w-full p-2 bg-white focus:outline-none focus:border-blue-500 text-gray-400 leading-4 transition-colors duration-200 ease-in-out'
    end
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-xs italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-grey-700 text-xs italic' }
  end
